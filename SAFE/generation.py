...
def long_form_generation_with_truth_value_api(
    ...
    generation_seed=None,
    context: str = "",
    batch_size: int = 1,  # default batch size to 1 for Instructor compatibility
    **kwargs
) -> dict:

    # If no question is provided, extract from the last user message
    if question is None:
        question = ""
    ...
    # Decompose generated text into claims
    print("Decomposing the generated text...")
    claims = decomp_method(generated_text)
    ...
    # Loop over each claim check method
    for claim_check_method in claim_check_methods:
        stmt_normalized_truth_values = []
        stmt_unnormalized_truth_values = []
        stmt_method_spec_outputs = []
        ...
        for i in range(0, len(claims), batch_size):
            ...
            for sidx, claim in enumerate(batch_claims):
                text_so_far = " ".join(claims[:i+sidx]) if (i+sidx) > 0 else None
