...
from bm25crossencode import obtain_docs
...
dataset_path = "factscore_bio.jsonl"

def get_datasetlocal(dataset: Union[str, list], size_of_data: int, seed: int = 0):
    # dataset is already a list → return as is
    if isinstance(dataset, list):
        return dataset

    # dataset is a string → treat as a path to the JSONL file
    return load_local_jsonl_dataset_fix(
        path=dataset_path,
        size_of_data=size_of_data,
        seed=seed
    )
...
#Our groups implementation!
def load_local_jsonl_dataset_fix(
    path: str,
    ...
    # sample if needed
    if size_of_data is not None and size_of_data < len(raw):
        random.seed(seed)
        raw = random.sample(raw, size_of_data)

    # now build final dataset WITH retrieval
    data = []
    for item in raw:
        context = "\n".join(obtain_docs(item["prompt"]))  # called only for sampled items
        print("questiont" + item["prompt"])
...
#i think here we need to load in context via RAG s